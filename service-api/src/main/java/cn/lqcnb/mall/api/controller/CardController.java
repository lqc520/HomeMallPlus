...
//        查询当前用户的购物车数据
    ...
    public R delete(@RequestHeader String token){
        return R.ok("ok",cardService.getSum(token));
    }
...
//        效验库存
        if(goodsService.getStock(cart.getGoodsId())<1){
            return R.error("库存不足 请联系管理员");
        }
...
//        检测购物车是否已经存在此商品
        ...
        if(curCart != null){
//           效验库存
            Integer stock = goodsService.getStock(cart.getGoodsId());
            Integer sum = Integer.parseInt(curCart.getNumber())+Integer.parseInt(cart.getNumber());
            Card params = new Card();
            params.setNumber(String.valueOf(sum > stock ? stock : sum));
//            更新购物车商品数量
            if(cardService.updateCount(params)){
                if(sum <= stock){
                    return R.ok("添加到购物车成功");
                }
                return R.ok("您好 货物快售空了 这是全部库存");
            }
            return R.error();
        }else{
            if(goodsService.cheStock(cart.getGoodsId(),Integer.parseInt(cart.getNumber()))){
