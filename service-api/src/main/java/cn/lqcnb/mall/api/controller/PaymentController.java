...
public class PaymentController {
    ...
    @PostMapping("/goToPay")
    public String goToPay(String out_trade_no,String total_amount,String subject) throws AlipayApiException, UnsupportedEncodingException {
        String res=paymentService.goPay(out_trade_no,total_amount,subject);
        logger.info("支付宝支付中",res);
        return res;
    }
    ...
    public void succeed(@RequestParam Map map, HttpServletResponse response, PrintWriter out){
        logger.info("支付宝","支付宝succeed成功！！！",map);
        if(map.size()!=0){
            //添加订单支付完成时间
            ...
            out.println("<html>");
            out.println("<script>");
            out.println("window.location.href="+baseUrl+"user.html'");
            out.println("</script>");
            out.println("</html>");
            out.flush();
            out.close();
    ...
    public void notifyPay(@ApiParam(hidden = true) @RequestParam Map map, @ApiParam(hidden = true) HttpServletResponse response, @ApiParam(hidden = true) PrintWriter out) throws UnsupportedEncodingException {
        logger.info("支付宝","支付宝notify成功！！！",map);
...
/*===========================IJPay==============================================================================*/
    @ApiImplicitParam(name = "day",value = "天数",paramType = "path",required = true)
    @ApiOperation("获取销售额度")
    @GetMapping("getSum/{day}")
    public BigDecimal getSum(@PathVariable Integer day){
        return paymentService.getSum(day);
    }
}
