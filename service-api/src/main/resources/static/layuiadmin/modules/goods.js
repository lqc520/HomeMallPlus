...
layui.define(['table', 'form'], function(exports){
  ...
  table.render({
    ...
    ,url:  setter.adminUrl+'goods/getList' //模拟接口
    ,cols: [[
  ...
  table.on('tool(LAY-goods-manage)', function(obj){
    ...
    if(obj.event === 'del'){
      layer.prompt({
        ...
        if(value=="lqcnb.cn"){
          layer.confirm('真的删除行么', function(index){
            $.ajax({
              url: setter.adminUrl+`goods/delete/${data.id}`,success:function (res) {
                layer.msg(res.msg);
              }
            });
            obj.del();
      ...
      layer.open({
        type: 2
        ,title: '编辑商品'
        ,content: '/admin/goods/goodsform.html?id='+data.id+"&name="+data.name
        ...
        ,yes: function(index, layero){
          ...
          iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
            ...
            //提交 Ajax 成功后，静态更新表格中的数据
             ...
             $.ajax({
               url: setter.adminUrl+'goods/update',
               data:field,
  ...
  table.render({
    ...
    ,url:  setter.adminUrl+'order/getOrderList' //模拟接口
    ,cols: [[
      {type: 'checkbox', fixed: 'left'}
  ...
  table.on('tool(LAY-order-manage)', function(obj){
    ...
    if(obj.event === 'del'){
      ...
      layer.open({
        type: 2
        ,title: '编辑订单'
        ,content: '/admin/goods/order/orderform.html?orderNumber='+data.order_number+"&orderStatus="+data.order_status
        ,maxmin: true
        ,area: ['500px', '350px']
        ,btn: ['确定', '取消']
        ,yes: function(index, layero){
          ...
          iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
            ...
            $.ajax({
              url: setter.adminUrl+'order/update',
              data:field,
              success:function (data) {
                layer.msg(data.msg);
              }
  ...
  table.render({
    elem: '#LAY-pay-manage'
    ,url:  setter.adminUrl+'pay/list' //模拟接口
    ,cols: [[
      {type: 'checkbox', fixed: 'left'}
      ,{field: 'id', width: 100, title: 'ID', sort: true}
      ...
      layer.open({
        ...
        ,yes: function(index, layero){
          ...
          iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
            ...
            $.ajax({
              url:base_admin_url+'order/update',
              data:field,
              success:function (data) {
                layer.msg(data.msg);
              }
